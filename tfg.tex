...
\newenvironment{resumen}{\vspace*{.3cm}\centerline{\large\bf Resumen}\vspace*{.5cm}}
...
In this work, we present an overview of different concepts related to complexity and polynomial approximation. First, we define some of the most important concepts about complexity, so we can study important concepts about aprroximability. Then, we present and analyze time approximation algorithms for some important NPO problems. Finally, we study how polynomial reductions relate to polynomial approximation to get more approximabilty results about NPO problems.



\vspace{2cm}
{\bf Keywords}: Computational complexity, approximation algorithms, polynomial approximation, approximability, polynomial reductions, NP-Complete problems.

\newpage

\begin{resumen}

En este trabajo estudiaremos diversos conceptos sobre complejidad y aproximaciones polin\'omicas. Primero definiremos algunos de los conceptos m\'as importantes sobre complejidad computacional para poder introducir conceptos importantes de aproximabilidad. Luego analizaremos la aproximabilidad de algunos problemas NPO importantes. Por \'ultimo estudiaremos c\'omo nos pueden ayudar las reducciones polin\'omicas para estudiar las aproximaciones polin\'omicas para conseguir m\'as resultados de aproximabilidad sobre los problemas NPO.
...
%Es f\'acil ver que la reducci\'on preserva los coeficientes de aproximaci\'on (con respecto al tamaño del grafo y al tamaño y valoraci\'on de los v\'ertices respectivamente). Pero, por otro lado, tambi\'en es f\'acil de ver que si el coeficiente dependiese de $f(\Delta(G))$ no habr\'ia una correspondencia 1 a 1, ya que $\Delta$ puede variar mucho si se aplica a $G$ o a $G'$, concretamente puede aumentar hasta $w_{max}$ (con $w_{max}=\{max\text{ }w_i | i=1...n\}$) veces, por tanto el coeficiente se transformar\'ia en un coeficiente $O(f(\Delta(G)w_{max}))$ para MAX PCIV.
